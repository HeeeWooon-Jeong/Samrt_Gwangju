...
select A.EMPLOYEE_ID
  ...
  from employees A
     , departments B
 WHERE SALARY BETWEEN 4500 AND 6000       -- A.SALARY == EMPLOYEES만 가지고 있는 자료이기 때문에 검색되지만
 -- FROM 써주는게 좋다
   AND A.DEPARTMENT_ID = B.DEPARTMENT_ID; -- DEPARTMENT_ID조건으로 추리기
...
select A.EMPLOYEE_ID
  ...
  from employees A INNER JOIN departments B
    ON  SALARY BETWEEN 4500 AND 6000
   AND A.DEPARTMENT_ID = B.DEPARTMENT_ID;

   --ANSI문법은 FROM 쪽에서 조인을 싹다 끝낸다
   -- ON 뒤에 조인조건 = 서로다른 테이블에서 컬럼 조건을 합치는것
...
SELECT a.department_id
     ...
     , B.FIRST_NAME
     , B.LAST_NAME
  FROM DEPARTMENTS A RIGHT OUTER JOIN EMPLOYEES B ON (a.manager_id = b.employee_id)
